...
public class MonthView extends View {
    ...
    public static final String VIEW_MULTI_SELECT_DAY = "VIEW_MULTI_SELECT_DAY"; // Multi model
    public static final String VIEW_RESERVATION_DAY = "VIEW_RESERVATION_DAY"; // 可预定的集合
    ...
    public static final String RESERVATION_DAY_TEXT = "RESERVATION_DAY_TEXT";
    public static final String RESERVATION_DAY_TEXT_COLOR = "RESERVATION_DAY_TEXT_COLOR";

    public static final int DEFAULT_TOP_TEXT_COLOR = Color.parseColor("#303133"); //#FF6E00
    ...
    public static final int DEFAULT_WEEKEND_TEXT_COLOR = Color.BLACK;  //#FF6E00
    public static final int DEFAULT_DIS_TEXT_COLOR = Color.parseColor("#BBBBBB");
    public static final int DEFAULT_SAME_TEXT_COLOR = Color.parseColor("#007AFF"); //#FF6E00
    ...
    public List<CalendarDay> multiSelectDays;
    private List<CalendarDay> reservationDays;
    ...
    private String reservationDayText;
    private int reservationDayTextColor;
    ...
    public MonthView(Context context, AttributeSet attrs, int defStyleAttr) {
        ...
        reservationDayText = (String) ATTRS.get(RESERVATION_DAY_TEXT);
        reservationDayTextColor = (int) ATTRS.get(RESERVATION_DAY_TEXT_COLOR);
    ...
    private void initPaint() {
        ...
        holidayPaint = new Paint(Paint.ANTI_ALIAS_FLAG);
        holidayPaint.setTextSize(topTextSize);
        holidayPaint.setColor(weekendTextColor);
        holidayPaint.setTextAlign(Paint.Align.CENTER);
    }
    ...
    private void drawDays(Canvas canvas) {
        ...
        for (int day = 1; day <= dayOfMonth; day++) {
            ...
            if (SelectionMode.MULTI == selectionMode) {
                if (isContainsInMultiSelectDay(day)) {
                    if (selectBgDrawable != null) {
                        selectBgDrawable.setBounds(dayRang.left + 4, dayRang.top, dayRang.right - 4, dayRang.bottom);
            ...
            if (SelectionMode.MULTI == selectionMode) {
                if (isContainsInMultiSelectDay(day)) {
                    ...
                    if (!TextUtils.isEmpty(firstSelectDayText)) {
...
//                    }
                    ...
                    if (isPreDay(day) || isMaxSelectDay(day) || isOverMaxRange(day)) {
                        ...
                        if (isSameDay(day)) {
                            ...
                            if (0 == dayOffset || columnNum - dayOffset == 1) {
                                ...
                                firstSelectPaint.setColor(weekendTextColor); // add
                            } else {
                                dayPaint.setColor(textColor);
                            }
                        }
                    ...
                    //加入假期逻辑
                    if (isShowTag()) {
                        String holiday = holidayMap.get(day);
                        if (!TextUtils.isEmpty(holiday)) {
//                        drawSelectDayText(canvas, holiday);
                            dayPaint.setColor(weekendTextColor);
                            firstSelectPaint.setColor(weekendTextColor);
                            canvas.drawText(holiday, dayRang.centerX(),
                                    dayRang.top + firstTopMargin + Math.abs(fm.ascent), holidayPaint);
                        }
                    }
                }
            } else {
...
//                        drawSelectDayText(canvas, holiday);
            ...
            if (isShowReservation()
                    && isContainsInReservationSelectDay(day)
                    && !TextUtils.isEmpty(reservationDayText)) {
                firstSelectPaint.setColor(reservationDayTextColor);
                ...
                canvas.drawText(reservationDayText, dayRang.centerX(), dayRang.top + firstTopMargin + firstHeight +
...
//                        dayRang.top + firstTopMargin + Math.abs(fm.ascent), firstSelectPaint);
                ...
                float daySecondDescent = fm.descent;
                float daySecondHeight = daySecondDescent - daySecondAscent;
                canvas.drawText(festival, dayRang.centerX(), dayRang.top + firstTopMargin + firstHeight +
                                secondTopMargin + daySecondHeight + thirdTopMargin + Math.abs(fm.ascent),
                        firstSelectPaint);
            }
            dayPaint.getFontMetrics(fm);
    ...
    private boolean isShowReservation() {
        return true;
    }
...
//                thirdPaint);
        ...
        canvas.drawText(selectDayText, dayRang.centerX(),
                dayRang.top + firstTopMargin + Math.abs(fm.ascent), thirdPaint);

    }

    private void drawDivider(Canvas canvas) {
//        canvas.drawRect(getPaddingLeft(), dayRang.bottom, getWidth() - getPaddingRight(), dayRang.bottom + dividerHeight, dividerPaint);
        canvas.drawRect(getPaddingLeft(), dayRang.bottom, dayWidth + dividerHeight, dayRang.bottom + dividerHeight, dividerPaint);
    }
     ...
     * @return
     */
    private boolean isOverMaxRange(int day) { //TODO
    ...
    public void setParamsMulti(Map<String, List> params) {
        if (params != null && params.containsKey(VIEW_MULTI_SELECT_DAY)) {
            multiSelectDays = params.get(VIEW_MULTI_SELECT_DAY);
        }
        requestLayout();
    }

    public void setParamsReservation(Map<String, List> params) {
        if (params != null && params.containsKey(VIEW_RESERVATION_DAY)) {
            reservationDays = params.get(VIEW_RESERVATION_DAY);
        }
        requestLayout();
    }
...
//                    lunar.lunarFestivalName : "";
            String festival = solar.isSFestival ? solar.solarFestivalName : lunar.isLFestival ?
                    lunar.lunarFestivalName : Lunar.getLunarDayString(lunar.lunarDay);
            if (!TextUtils.isEmpty(festival)) {
                //todo 去掉农历
//                festivalMap.put(day, festival);
            }
            String holiday = LunarSolarConverter.getHoliday(month + 1, day);
            if (!TextUtils.isEmpty(holiday)) {

                //todo 去掉节假日
//                holidayMap.put(day, holiday);
            }
    ...
    private boolean isContainsInMultiSelectDay(int day) {
        if (multiSelectDays == null) {
            return false;
        }
        for (CalendarDay calendarDay : multiSelectDays) {
            if (year == calendarDay.getYear()
    ...
    private boolean isContainsInReservationSelectDay(int day) {
        if (reservationDays == null) {
            return false;
        }
        for (CalendarDay calendarDay : reservationDays) {
            if (year == calendarDay.getYear()
                    && month == calendarDay.getMonth()
                    && day == calendarDay.getDay()) {
                return true;
