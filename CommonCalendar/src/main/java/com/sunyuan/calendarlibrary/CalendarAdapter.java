...
import static com.sunyuan.calendarlibrary.MonthView.RESERVATION_DAY_TEXT;
import static com.sunyuan.calendarlibrary.MonthView.RESERVATION_DAY_TEXT_COLOR;
 ...
 */
public final class CalendarAdapter extends RecyclerView.Adapter<CalendarAdapter.CalendarHolder> implements
    ...
    private void parseTypedArray(Context context, TypedArray typedArray) {
        ...
        int selectTextColor =
            typedArray.getColor(R.styleable.CalendarView_cl_selectTextColor, DEFAULT_SELECT_TEXT_COLOR);
        Drawable selectBgDrawable = typedArray.getDrawable(R.styleable.CalendarView_cl_selectBgDrawable);
        int weekendTextColor =
            typedArray.getColor(R.styleable.CalendarView_cl_weekendTextColor, DEFAULT_WEEKEND_TEXT_COLOR);
        ...
        int topTextSize =
            (int)typedArray.getDimension(R.styleable.CalendarView_cl_topTextSize, Utils.sp2px(context, 10));
        ...
        int bottomTextSize =
            (int)typedArray.getDimension(R.styleable.CalendarView_cl_bottomTextSize, Utils.sp2px(context, 10));
        ...
        int selectMaxRange =
            typedArray.getInteger(R.styleable.CalendarView_cl_selectMaxRange, MonthView.DEFAULT_SELECT_MAX_RANGE);
        int dividerHeight =
            (int)typedArray.getDimension(R.styleable.CalendarView_cl_dividerHeight, MonthView.DEFAULT_DIVIDER_HEIGHT);
        int dividerColor =
            typedArray.getColor(R.styleable.CalendarView_cl_dividerColor, MonthView.DEFAULT_DIVIDER_COLOR);
        ...
        String reservationDayText = typedArray.getString(R.styleable.CalendarView_cl_reservationDayText);
        int reservationDayTextColor =
            typedArray.getColor(R.styleable.CalendarView_cl_reservationDayTextColor, MonthView.DEFAULT_DIVIDER_COLOR);
        ...
        ATTRS.put(RESERVATION_DAY_TEXT, reservationDayText);
        ATTRS.put(RESERVATION_DAY_TEXT_COLOR, reservationDayTextColor);
    }
    ...
    public void onBindViewHolder(@NonNull CalendarHolder calendarHolder, int position) {
        ...
        if (SelectionMode.MULTI == selectionMode) {
            Map<String, List> parmasMulti = new HashMap<>();
            parmasMulti.put(MonthView.VIEW_MULTI_SELECT_DAY, calendarSelectDay.getMultiSelectDay());
            calendarHolder.monthView.setParamsMulti(parmasMulti);
        }
        if (calendarSelectDay.getReservationDay() != null && calendarSelectDay.getReservationDay().size() > 0) {
            Map<String, List> parmasReservation = new HashMap<>();
            parmasReservation.put(MonthView.VIEW_RESERVATION_DAY, calendarSelectDay.getReservationDay());
            calendarHolder.monthView.setParamsReservation(parmasReservation);
        }
    }
    ...
    public void onDayClick(CalendarDay calendarDay) {
        if (onCalendarSelectDayListener != null) {
            ...
            } else if (SelectionMode.MULTI == selectionMode) { //
                // TODO
                if (isContainsInMultiDay(calendarDay)) {
                } else {
                    calendarSelectDay.getMultiSelectDay().add(calendarDay);
                }
            } else {
                CalendarDay firstSelectDay = calendarSelectDay.getFirstSelectDay();
                if (firstSelectDay != null) {
    ...
    private boolean isContainsInMultiDay(CalendarDay calendarDayInput) {
        Iterator<CalendarDay> it = calendarSelectDay.getMultiSelectDay().iterator();
        while (it.hasNext()) {
            CalendarDay calendarDay = it.next();
            if (calendarDayInput.getYear() == calendarDay.getYear()
                && calendarDayInput.getMonth() == calendarDay.getMonth()
                && calendarDayInput.getDay() == calendarDay.getDay()) {
                it.remove();
                return true;
            }
        }

        // for (CalendarDay calendarDay : calendarSelectDay.getMultiSelectDay()) {
        // if (calendarDayInput.getYear() == calendarDay.getYear()
        // && calendarDayInput.getMonth() == calendarDay.getMonth()
        // && calendarDayInput.getDay() == calendarDay.getDay()) {
        // calendarSelectDay.getMultiSelectDay().remove(calendarDay);
        // return true;
        // }
        // }
        return false;
    ...
    void init(CalendarViewWrapper.CalendarBuilder calendarBuilder) {
        ...
        Date minDate = calendarBuilder.minDate;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.KITKAT) {
            minDate = Objects.requireNonNull(calendarBuilder.minDate);
        }
        Date maxDate = calendarBuilder.maxDate;
        if (android.os.Build.VERSION.SDK_INT >= android.os.Build.VERSION_CODES.KITKAT) {
            maxDate = Objects.requireNonNull(calendarBuilder.maxDate);
        }
